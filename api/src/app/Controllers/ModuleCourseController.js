...
class ModuleCourseController {
    ...
    async index(req, res) {
        ...
        await ModuleCourse.paginate(query, options)
            .then((response) => {
    ...
    async store(req, res) {
        ...
        const module = await ModuleCourse.create(dados, (err) => {
            if (err)
                return res.status(400).json({
                    error: true,
                    code: 101,
...
module.exports = new ModuleCourseController();